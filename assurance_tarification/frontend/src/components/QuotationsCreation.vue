...
const onSubmit = async (event: FormSubmitEvent<Quote>) => {
  ...
  const newQuote = await mutation.mutateAsync(event.data)
  downloadFile(newQuote.id, 'word', newQuote)
  downloadFile(newQuote.id, 'pdf', newQuote)
  router.push('/')
}
...
<template>
  ...
  <UForm :state="state" @submit="onSubmit">
    <div class="grid grid-cols-3 gap-8 min-w-64">
      <div class="flex flex-col gap-4">
        <h2 class="text-xl font-bold mb-4">Informations sur le client</h2>
        <UFormField label="Numéro d'opportunité" name="num_opportunite" required>
          <UInput v-model="state.num_opportunite" class="w-full" />
        </UFormField>

        <UFormField label="Nom du Client" name="nom_client" required>
          <UInput v-model="state.nom_client" class="w-full" />
        </UFormField>
      ...
      <div class="flex flex-col gap-4">
        ...
        <UFormField label="Type de Garantie" name="type_garantie" required>
          <USelect v-model="state.type_garantie" :items="garanties" class="w-full" />
        </UFormField>

        <UFormField label="Type de Bien" name="type_bien" required>
          <USelect v-model="state.type_bien" :items="biens" class="w-full" />
        </UFormField>

        <UFormField label="Type de Travaux" name="type_travaux" required>
          <USelect v-model="state.type_travaux" :items="travaux" class="w-full" />
        </UFormField>

        <UFormField label="Coût de l'Ouvrage" name="tarif_propose" required>
